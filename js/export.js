...
export async function exportarExcel(txs, groupMembers, userId, opciones) {
  ...
  const txsFiltradas = filtrarPorRango(txs, desde, hasta).filter((t) =>
    tipos.includes(t.type),
  );
  ...
  const resumenData = [
    ...
    [
      "Período",
      `${MONTHS[desde.getMonth()]} ${desde.getFullYear()} — ${MONTHS[hasta.getMonth()]} ${hasta.getFullYear()}`,
    ],
    [""],
    ["Concepto", "Monto"],
  ...
  const filas = txsFiltradas.map((t) => {
    const pagador =
      t.payer_id === userId ? "Yo" : nombreMiembro(t.payer_id, groupMembers);
    return [
  ...
  if (desde.getTime() !== hasta.getTime()) {
    ...
    const mesFila = [
      ["Mes", "Ingresos", "Gastos", "Compartido", "Ahorros", "Saldo"],
    ];
...
function agruparPorMes(txs, userId) {
  ...
  txs.forEach((t) => {
    ...
    const r = meses[key];
    if (t.type === "income" && t.user_id === userId)
      r.ingresos += Number(t.amount);
    else if (t.type === "expense" && t.user_id === userId)
      r.gastos += Number(t.amount);
    else if (t.type === "shared") {
      const iAmPayer = t.payer_id === userId;
      const involved =
        t.user_id === userId ||
