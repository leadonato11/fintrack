...
// GRUPO
// ============================================
async function cargarGrupo(esRegistroNuevo = false) {
  const g = await getUserGroup(state.user.id);

  if (g) {
    // Usuario ya tiene grupo — solo cargamos los datos
    state.group = g;
    state.user.groupId = g.group_id;
    const members = await getGroupMembers(g.group_id);
    state.groupMembers = members;
  } else if (esRegistroNuevo) {
    // Solo creamos grupo nuevo si es un registro nuevo
    // nunca en un login normal
  ...
  // Si no tiene grupo y no es registro nuevo, no hacemos nada
  // El usuario va a ver el perfil vacío hasta que lo inviten
}
...
function renderIncome() {
  ...
  document.getElementById("incList").innerHTML = txs.length
    ...
    : empty("No hay ingresos este mes");
}
...
function renderSavings() {
  ...
  document.getElementById("svList").innerHTML = svTxs.length
    ? svTxs.map((t) => txCard(t, "saving")).join("")
    : empty("Marcá un ahorro con el botón +");
}
  ...
  document.getElementById("uList").innerHTML = state.groupMembers.length
    ? state.groupMembers
        ...
        .map((member) => {
          ...
          <span class="ubadge ${esYo ? "bme" : "bother"}">${
            esYo ? "Vos" : "Grupo"
          }</span>
        </div>`;
        })
...
const CAT_SVG = {
  ...
  servicios:
    '<path d="M9 18H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h4"/><polyline points="12 10 17 6 12 2"/><line x1="17" y1="6" x2="17" y2="20"/><line x1="21" y1="15" x2="13" y2="15"/>',
...
function txCard(t, type) {
  ...
  const color =
    type === "income" ? "#0F6E56" : type === "saving" ? "#185FA5" : "#D85A30";
  const sign = type === "income" ? "+" : type === "saving" ? "" : "−";
  const vc =
    type === "income" ? "v-green" : type === "saving" ? "v-blue" : "v-red";
...
document.getElementById("pct1")?.addEventListener("input", function () {
  document.getElementById("pct2").value =
...
window.doInvite = async function () {
  ...
  if (!email) {
    err.textContent = "Ingresá un email";
    return;
  }
  if (!state.user?.groupId) {
    err.textContent = "No tenés un grupo creado";
    return;
  }
